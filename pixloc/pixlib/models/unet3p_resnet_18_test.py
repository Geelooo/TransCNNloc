...
class UNet3Plus(nn.Module):
    ...
    # def _init(self, conf):
    def __init__(self,  n_classes=1, bilinear='bilinear', feature_scale=4,
                 is_deconv=True):
        super().__init__()
        ...
        # hd5->24*32, hd4->48*64
        self.hd5_UT_hd4 = nn.Upsample(scale_factor=2, mode=self.bilinear, align_corners=False)  # 14*14
        ...
        self.hd4_UT_hd3 = nn.Upsample(scale_factor=2, mode=self.bilinear, align_corners=False) 
        self.hd4_UT_hd3_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd4_UT_hd3_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd4_UT_hd3_relu = nn.ReLU(inplace=True)

        # hd5->24*32, hd4->96*128
        self.hd5_UT_hd3 = nn.Upsample(scale_factor=4, mode=self.bilinear, align_corners=False) 
        ...
        self.hd3_UT_hd2 = nn.Upsample(scale_factor=2, mode=self.bilinear, align_corners=False)
        self.hd3_UT_hd2_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd3_UT_hd2_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd3_UT_hd2_relu = nn.ReLU(inplace=True)

        # hd4->48*64, hd2->192*256
        self.hd4_UT_hd2 = nn.Upsample(scale_factor=4, mode=self.bilinear, align_corners=False) 
        self.hd4_UT_hd2_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd4_UT_hd2_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd4_UT_hd2_relu = nn.ReLU(inplace=True)

        # hd5->24*32, hd2->192*256
        self.hd5_UT_hd2 = nn.Upsample(scale_factor=8, mode=self.bilinear, align_corners=False) 
        ...
        self.hd2_UT_hd1 = nn.Upsample(scale_factor=2, mode=self.bilinear, align_corners=False)
        self.hd2_UT_hd1_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd2_UT_hd1_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd2_UT_hd1_relu = nn.ReLU(inplace=True)

        # hd3->96*128, hd1->384*512
        self.hd3_UT_hd1 = nn.Upsample(scale_factor=4, mode=self.bilinear, align_corners=False)
        self.hd3_UT_hd1_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd3_UT_hd1_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd3_UT_hd1_relu = nn.ReLU(inplace=True)

        # hd4->48*64, hd1->384*512
        self.hd4_UT_hd1 = nn.Upsample(scale_factor=8, mode=self.bilinear, align_corners=False)
        self.hd4_UT_hd1_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd4_UT_hd1_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd4_UT_hd1_relu = nn.ReLU(inplace=True)

        # hd5->24*32, hd1->384*512
        self.hd5_UT_hd1 = nn.Upsample(scale_factor=16, mode=self.bilinear, align_corners=False)
        ...
        self.outconv2 = nn.Conv2d(320, 64, 1)
        self.outconv3 = nn.Conv2d(320, 128, 1)

        # uncertainty
        self.uncer1=nn.Conv2d(320,1,1,1)
        self.uncer2=nn.Conv2d(320,1,1,1)
        self.uncer3=nn.Conv2d(320,1,1,1)
        ...
        # level-1
        self.block1 = nn.Sequential(model1.conv1,model1.bn1,model1.relu)
        # level-2
        self.block2 = nn.Sequential(model1.maxpool,model1.layer1)
        # level-3
        self.block3 = model1.layer2
        # level-4
        self.block4 = model1.layer3
        # level-5
        self.block5 = model1.layer4
    ...
    def forward(self, inputs):
        ...
        feature_map_3 = self.outconv2(hd2)  # feature3->128*96*128
        feature_map_5 = self.outconv3(hd4)  # feature5->128*24*32

        uncertainty_map1=self.uncer1(hd1)   # un1->1*384*512
        uncertainty_map3=self.uncer2(hd2)   # un3->1*96*128
        uncertainty_map5=self.uncer3(hd4)   # un5->1*24*32
