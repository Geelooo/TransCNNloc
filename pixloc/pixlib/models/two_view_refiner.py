...
class TwoViewRefiner(BaseModel):
    ...
    def _init(self, conf):
        self.extractor = get_model(conf.extractor.name)(conf.extractor)# 网络模型的导入和读取
        assert hasattr(self.extractor, 'scales')

        Opt = get_model(conf.optimizer.name)
        if conf.duplicate_optimizer_per_scale:
            oconfs = [deepcopy(conf.optimizer) for _ in self.extractor.scales]#其实没什么用只是用来关心复制了几次
    ...
    def _forward(self, data):
        ...
        pred['T_r2q_opt'] = []
        pred['valid_masks'] = []
        for i in reversed(range(len(self.extractor.scales))):  # i->2,1,0
    ...
    def loss(self, pred, data):
        ...
        p2D_q_gt, mask = project(data['T_r2q_gt'])     # 将512个ref共视点利用query的RT投影到query中
        p2D_q_i, mask_i = project(data['T_r2q_init'])  # 用自定义的初始值投影，默认为E|0
        mask = (mask & mask_i).float()

        too_few = torch.sum(mask, -1) < 10
        if torch.any(too_few):    # 用于判断是否有true项
            logger.warning(
        ...
        def reprojection_error(T_r2q):
            p2D_q, _ = project(T_r2q)   # p2D_q和p2D_q_gt [512,2]  
            err = torch.sum((p2D_q_gt - p2D_q)**2, dim=-1)  # err [512,1]
        ...
        for i, T_opt in enumerate(pred['T_r2q_opt']):
            err = reprojection_error(T_opt).clamp(max=self.conf.clamp_error) # 钳制最大误差在50以下
            loss = err / num_scales
            if i > 0:
    ...
    def metrics(self, pred, data):
        T_q2r_gt = data['ref']['T_w2cam'] @ data['query']['T_w2cam'].inv() # 求逆操作
