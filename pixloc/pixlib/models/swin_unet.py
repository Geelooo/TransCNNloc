...
class SwinUnet(BaseModel):
    ...
    def _forward(self, data):
        ...
        image = (image - mean[:, None, None]) / std[:, None, None]    # 对照片进行归一化处理
        x = image
        ...
        uct_map = []
        uct_map.append(torch.sigmoid(-self.utc3(feature_map[0])))
        uct_map.append(torch.sigmoid(-self.utc2(feature_map[1])))
        # uct_map.append(torch.sigmoid(-self.utc1(feature_map[2])))
        uct_map.append(torch.sigmoid(-self.utc0(feature_map[2])))
