...
class UNet3Plus(BaseModel):
    ...
    def _init(self, conf):
        ...
        self.outconv3 = nn.Conv2d(filters[-1], 128, 1)

        # uncertainty
        self.uncer1=nn.Conv2d(320,1,1,1)
        self.uncer2=nn.Conv2d(320,1,1,1)
        self.uncer3=nn.Conv2d(filters[-1],1,1,1)
        ...
        # level-1
        self.block1 = torch.nn.Identity()
        # level-2
        self.block2 = nn.Sequential(model1.conv1,model1.bn1,model1.relu)
        # level-3
        self.block3 = nn.Sequential(model1.maxpool,model1.layer1)
        # level-4
        self.block4 = model1.layer2
        # level-5
        self.block5 = model1.layer3
    ...
    def _forward(self, data):
        ...
        # inputs 3*384*512
        h1 = self.block1(image)  # h1->3*384*512

        h2 = self.block2(h1)  # h2->64*192*256

        h3 = self.block3(h2)  # h3->64*96*128

        h4 = self.block4(h3)  # h4->128*48*64
 
        hd5 = self.block5(h4)  # h5->256*24*32
