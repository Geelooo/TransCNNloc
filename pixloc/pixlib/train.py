...
def training(rank, conf, output_dir, args):
    ...
    # torch.autograd.set_detect_anomaly(True)    # 为 autograd 引擎启用异常检测
    ...
    lr_params = pack_lr_parameters(
            params, conf.train.lr, conf.train.lr_scaling) # 将参数分组，不同参数的利用不同的学习率
    ...
    while epoch < conf.train.epochs and not stop:
        ...
        set_seed(conf.train.seed + epoch)#只要初始种子一样后面就都一样
        if args.distributed:
        ...
        for it, data in enumerate(train_loader):
            ...
            if do_backward:
                loss.backward() # 把total的损失值往回传递
                optimizer.step()
                lr_scheduler.step()
                if conf.train.get('clip_grad', None):# 梯度截断 改一下阈值 判断有多少梯度是超过1的 超过的按照clip_grad减掉
            ...
            if it % conf.train.log_every_iter == 0:   # 每隔多少次迭代记录一次
                for k in sorted(losses.keys()):# 将所有pair的不同层次的pair平均一下
                    if args.distributed:
                ...
                if rank == 0:
                    ...
                    logger.info('[E {} | it {}] loss {{{}}}'.format(
                        epoch, it, ', '.join(str_losses)))# 打印输出平均后各层的损失
                    for k, v in losses.items(): # 将结果写到tensorboard中
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore', action='store_true')# 应该是重新开始还是继续上次的权重
    parser.add_argument('--distributed', action='store_true')# 是否利用多显卡进行训练
    ...
    if args.conf:# 训练配置yaml文件的导入
        conf = OmegaConf.merge(OmegaConf.load('/home/lys/Workplace/python/pixloc/'+args.conf), conf)
    if not args.restore:
