...
def load_experiment(exper, conf={}, get_last=False):
    ...
    ckpt = torch.load(str(ckpt), map_location='cpu')# 包含特征提取和预测的所有权重和偏置
                                                    # 为啥conf也在里面叻
    ...
    model = get_model(conf.name)(conf).eval()  # 下载vgg19.pth到cache中
            # 按照conf.name导入类后，利用conf进行构造并将网络打到评估模式
    ...
    model.load_state_dict(state_dict, strict=False) # 将预训练的权重加载到新模型
    return model
