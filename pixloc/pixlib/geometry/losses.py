...
def barron_loss(x, alpha, derivatives: bool = True, eps: float = 1e-7):
    ...
    beta_safe = torch.abs(alpha - 2.).clamp(min=eps)  # 将|2-alpha|的最小值牵制在eps处
    ...
    loss = torch.where(
        alpha == 0, loss_zero,
        torch.where(alpha == 2, loss_two, loss_otherwise)) # 训练中实际上等于loss_otherwise
    ...
    if derivatives:
        ...
        loss_d1 = torch.where(
            alpha == 0, loss_zero_d1,# 训练中实际上等于loss_otherwise_d1
