...
class DirectAbsoluteCost:
    ...
    def residuals(
        ...
        p3D_q = T_w2q * p3D  # 3D 点在相机系下的坐标
        p2D, visible = camera.world2image(p3D_q) # 在图像上的位置
        F_p2D_raw, valid, gradients = self.interpolator(
            F_query, p2D, return_gradients=do_gradients)  #把query的残差差值到跟参考图片点数量一样多
        valid = valid & visible
        ...
        if confidences is not None:
            ...
            # weight = C_query_p2D
            # weight = torch.where(C_ref>C_query_p2D,C_ref,C_query_p2D)
    ...
    def jacobian(
        ...
        J_p3D_T = T_w2q.J_transform(p3D_q)  # 相机坐标系3D点
        J_p2D_p3D, _ = camera.J_world2image(p3D_q)  # F对p求偏导

        if self.normalize:
            J_f_p2D = J_normalization(F_p2D_raw) @ J_f_p2D

        J_p2D_T = J_p2D_p3D @ J_p3D_T   # p对δ求偏导
        J = J_f_p2D @ J_p2D_T
        return J, J_p2D_T
    ...
    def residual_jacobian(
        ...
        J, _ = self.jacobian(T_w2q, camera, *info) # 外参 内参 相机系坐标 差值后的残差 梯度？
